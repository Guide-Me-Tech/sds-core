...
Example:
    python sds_core/utils/generate_jsonschema.py doc.document.TableCell
...
def generate_json_schema(class_reference: str) -> Union[dict, None]:
    ...
    Args:
        class_reference: The reference to a class in 'sds_core.types'.

    Returns:
        A jsonable dict of the model's schema.
    """
    if not class_reference.startswith("sds_core.types."):
        class_reference = "sds_core.types." + class_reference
