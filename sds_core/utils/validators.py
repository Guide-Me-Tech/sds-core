...
def validate_raw_schema(file_: dict) -> tuple[bool, str]:
    ...
    schema_txt = resources.files("sds_core").joinpath("resources/schemas/legacy_doc/RAW.json").read_text("utf-8")
...
def validate_ann_schema(file_: dict) -> tuple[bool, str]:
    ...
    schema_txt = resources.files("sds_core").joinpath("resources/schemas/legacy_doc/ANN.json").read_text("utf-8")
    schema = json.loads(schema_txt)
