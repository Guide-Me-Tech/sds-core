...
def run_dump(cfg: dict[str, Any]) -> int:
    ...
    - dataset_name: str ("sds-project/doclaynet-set-a")
    ...
    """
    dataset_name = cfg.get("dataset_name", "sds-project/doclaynet-set-a")
...
def run_analyse(cfg: dict[str, Any]) -> int:
    ...
    Config keys (with defaults):
    - tokenizer_name: str ("ibm-granite/granite-sds-258M")
    - input_glob: str ("./scratch/doclang/*_with_h.dclg")
    - pair_replace: dict with {"from": "_h", "to": ""} to locate paired files (optional)
    - show_plots: bool (True)
    - verbose: bool (False)
    """
    tokenizer_name = cfg.get("tokenizer_name", "ibm-granite/granite-sds-258M")
...
def default_config(mode: str) -> dict[str, Any]:
    if mode == "dump":
        return {
            "dataset_name": "sds-project/doclaynet-set-a",
        ...
        return {
            "tokenizer_name": "ibm-granite/granite-sds-258M",
